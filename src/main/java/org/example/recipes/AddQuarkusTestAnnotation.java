...
public class AddQuarkusTestAnnotation extends Recipe {
    ...
    public static class QuarkusTestAnnotationVisitor extends JavaIsoVisitor<ExecutionContext> {
        ...
        private final JavaTemplate componentAnnotationTemplate = JavaTemplate.builder(this::getCursor, "@QuarkusTest").javaParser(() -> JavaParser.fromJavaVersion().classpath("quarkus-junit5").build()).imports(IMPORT_COMPONENT).build();

        @Override
        public J.ClassDeclaration visitClassDeclaration(J.@NotNull ClassDeclaration classDecl, @NotNull ExecutionContext executionContext) {
            ...
            if (cd.getAllAnnotations().stream().noneMatch(new AnnotationMatcher("@QuarkusTest")::matches)) {
                ...
                cd = cd.withTemplate(componentAnnotationTemplate, cd.getCoordinates().addAnnotation(Comparator.comparing(J.Annotation::getSimpleName))); //Annotation is added but not the import
